...
for (i in 1:1) {
  ...
  fname <- paste0(stusab,"_wac_S000_JT00")
  # md5sum new name for LODES7.1 - possible to compare to old file
  panelname <- paste0("L:/LODESPanelTest/",fname, ".csv")
  ...
  filenames <- list.files(path = ".",pattern = "*\\_wac_S000_JT00_.*\\.csv$")
  ...
  read_csv_filename <- function(filename){
    ...
    ret <- read.csv(filename, header = TRUE, colClasses = classes)
