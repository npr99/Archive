...
%MACRO MakeGraphs(dfile,title1,JobVar,yaxis);
* For residential area characteristics files;
%IF &DFile = RAC %THEN %LET DataFile = Residence Area Characteristic data;
%IF &DFile = RAC %THEN %LET DfileFootnote = jobs are totaled by home Census Block;
* For worker area characteristics files;
%IF &DFile = WAC %THEN %LET DataFile = Workplace Area Characteristic data;
%IF &DFile = WAC %THEN %LET DfileFootnote = jobs are totaled by work Census Block;

%MACRO MakeGraph(title1,title2,JobVar,cbsaid,LRTOpen_dates,hrefvaluelist,yaxis,min,max,by);
/* Define a title for the graph */
title1 height = 14pt justify = c ls=0.5 "LODES &DataFile by &title1";
title2 height = 12pt justify = c ls=0.5 "by distance from nearest LRT Station for &title2";
...
footnote2 height = 8pt justify = l ls=0.5 "I_&JobVar.&JobType from data=&library..WLLRT_&cbsaid.&Dfile.&SegType.&JobType._index";
footnote3 height = 8pt justify = l ls=0.5 "&DfileFootnote";

   proc gplot data=&library..WLLRT_&cbsaid.&Dfile.&SegType.&JobType._index;
...
%MakeGraph(&title1,Austin,&JobVar,12420,22 Mar 2010,"22MAR2010"d,&yaxis,80,180,20);
*%MakeGraph(&title1,Los Angeles,&JobVar,31100,26 July 2003 15 Nov 2009 28 Apr 2012 20 Jun 2012,15912 18216 19111 19164,&yaxis,80,180,20);
*%MakeGraph(&title1,Dallas,&JobVar,19100,1 July 2002 9 Dec 2002 14 Sep 2009 6 Dec 2010 30 Jul 2012,15522 15683 18154 18602 19204,&yaxis,80,180,20);
*%MakeGraph(&title1,Charlotte,&JobVar,16740,24 Nov 2007,"24Nov2007"d,&yaxis,80,180,20);
...
ods pdf file="F:\Dropbox\MyProjects\WLLRT\Work\WLLRT_DataExplore_DonutRanges_2015-09-30_&Dfile.&SegType.&JobType._index.pdf" ;


%let yaxis = Index of  Primary Private Jobs (2002 = 100);
%let title1 = Primary Private Jobs;
%let JobVar = C000;
%MakeGraphs(&DFile,&title1,&JobVar,&yaxis);
