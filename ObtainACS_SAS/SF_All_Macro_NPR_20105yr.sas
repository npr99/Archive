...
%LET F_seqnum = 10; /*First sequence number to load */
%LET L_seqnum = 10; /*Last sequence number to load */
...
%LET dd_SASLib = C:\Users\Nathanael99\Dropbox\MyData\Census\ACS\acs2010_5yr\;
...
data _null_;
  ...
  /*Save code to FILE statement below*/
	...
	if order =1     then put " ";
	/* Retain the Name of the Subsample to put in title of variable */
	/* The Subsample ends with ":"*/
	if find(title,":") > 1 AND order ^= . then do;
		retain subsample; 
		subsample = title;
	end; 
	...
	if order ^=. then	 do;
		lineout= compress(tblid)||"&type"||compress(order)||" format = comma16. label ='"||trim(title)||" "||trim(universe)||" "||trim(subsample)||"(&type)'";
	    put	lineout;
	end;
run;
