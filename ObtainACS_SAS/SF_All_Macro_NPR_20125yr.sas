...
/* geographies from the 2012 5yr ACS summary file.                   */
...
/*                                                                   */
/*-------------------------------------------------------------------*/
/* Date Last Updated: 30 Sept 2014                                   */
/*-------------------------------------------------------------------*/
...
%LET dd_SASLib = C:\Users\Gabe\Documents\MySASLib\;
/* Location where ReadDataFile Macro will save sas code */
%LET Include_Prog = C:\Users\Gabe\Documents\Syllabus\TAMU\Research\Dissertation\Summary_Files\Census_ACS_2012_5yr\ACS_2012_SF_Code\;
/* Location for state level sequence files */
%LET dd_ACSSeqFiles = C:\Users\Gabe\Documents\Syllabus\TAMU\Research\Dissertation\Summary_Files\Census_ACS_2012_5yr\2008-2012_ACSSF_All\Tracts_Block_Groups_Only\tab4\sumfile\prod\2008thru2012\group2\;
/* Loction for Geography files */
%LET dd_Geography_files = C:\Users\Gabe\Documents\Syllabus\TAMU\Research\Dissertation\Summary_Files\Census_ACS_2012_5yr\2008-2012_ACSSF_All\2012_ACS_Geography_Files\;
...
%LET F_seqnum = 1; /*First sequence number to load */
%LET L_seqnum = 120; /*Last sequence number to load */
%LET F_state = 1; /*First state to load */
%LET L_state = 72; /*Last state to load */
/*-------------------------------------------------------------------*/
/* Define SAS Library                                                */
/*-------------------------------------------------------------------*/
...
data work.&geography;
  ...
  INFILE "&dd_Geography_files.&geography..txt" MISSOVER TRUNCOVER LRECL=1500;
...
data _null_;
  ...
  /*Save code to FILE statement below*/
	...
	if _n_ =1 then 		 do;
		...
		put "		   LOGRECNO $7;";
		put " ";
		put "INFILE '&dd_ACSSeqFiles.&type.&year.5&geo.&seq.000.txt' DSD TRUNCOVER DELIMITER =',' LRECL=3000;";
		put " ";
...
data _null_;
  set work.Seq_&seq;
   FILE "&rootdir.ReorderEM&geo._&seq..sas";
	if _n_ =1 then 		 do;
   		put "TITLE ""E and M &year.5&geo.&seq.000"";";
...
EDIT COMMENT OUT*/
    ...
    data acs.SF&seq&geo;
  	  merge  g&year.5&geo(IN=g) SFem&seq&geo(IN=x);
   		by logrecno;
 	run;
